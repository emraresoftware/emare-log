...
<div class="container-fluid">
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">Mikrotik Düzenle</li>
        </ol>
    </nav>

    <h1 class="h3 mb-4">Mikrotik Düzenle: {{ $mikrotik->ad }}</h1>

    <form method="POST" action="{{ route('mikrotik.update', $mikrotik->id) }}">
        @csrf
        @method('PUT')
            ...
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-4">
                        ...
                        <input type="text" name="ad" class="form-control @error('ad') is-invalid @enderror" value="{{ old('ad', $mikrotik->ad) }}" required>
                        @error('ad')
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" name="ip_adresi" class="form-control @error('ip_adresi') is-invalid @enderror" value="{{ old('ip_adresi', $mikrotik->ip_adresi) }}" required>
                        @error('ip_adresi')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-md-4">
                        ...
                        <input type="number" name="api_port" class="form-control @error('api_port') is-invalid @enderror" value="{{ old('api_port', $mikrotik->api_port) }}" required>
                        @error('api_port')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-md-4">
                        ...
                        <input type="text" name="kullanici_adi" class="form-control @error('kullanici_adi') is-invalid @enderror" value="{{ old('kullanici_adi', $mikrotik->kullanici_adi) }}" required>
                        @error('kullanici_adi')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-md-4">
                        ...
                        <div class="input-group">
                            <input type="password" name="sifre" id="sifreInput" class="form-control" placeholder="Değiştirmek için yeni şifre giriniz">
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="bolge_id" class="form-select">
                            ...
                            @foreach($bolgeler ?? [] as $bolge)
                                <option value="{{ $bolge->id }}" {{ old('bolge_id', $mikrotik->bolge_id) == $bolge->id ? 'selected' : '' }}>{{ $bolge->ad }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-8">
                        <label class="form-label">Açıklama</label>
                        <input type="text" name="aciklama" class="form-control" value="{{ old('aciklama', $mikrotik->aciklama) }}">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Durum <span class="text-danger">*</span></label>
                        <select name="durum" class="form-select" required>
                            <option value="aktif" {{ old('durum', $mikrotik->durum) == 'aktif' ? 'selected' : '' }}>Aktif</option>
                            <option value="pasif" {{ old('durum', $mikrotik->durum) == 'pasif' ? 'selected' : '' }}>Pasif</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="card-body">
                <div class="d-flex align-items-center justify-content-between">
                    <div>
                        ...
                        <p class="text-muted mb-0">Güncelleme öncesi bağlantıyı test edebilirsiniz.</p>
                    </div>
        ...
        <div class="d-flex justify-content-end gap-2 mb-4">
            ...
            <button type="submit" class="btn btn-primary">
                <i class="fas fa-save me-1"></i> Mikrotik Güncelle
            </button>
        </div>
...
<script>
    ...
    function baglantiTest() {
        ...
        if (!ip || !port || !kullanici) {
            toastr.warning('Lütfen IP adresi, port ve kullanıcı adı alanlarını doldurunuz.');
            return;
        }
