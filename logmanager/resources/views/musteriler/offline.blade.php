...
<div class="page-header d-flex justify-content-between align-items-center mb-4">
    <div>
        <h1 class="page-title">Offline Müşteriler</h1>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">Offline Müşteriler</li>
            </ol>
        </nav>
    </div>
</div>
...
<div class="card mb-4">
    <div class="card-body">
        <form method="GET" action="{{ route('musteriler.offline') }}">
            <div class="row g-3 align-items-end">
                ...
                <div class="col-md-4">
                    ...
                    <a href="{{ route('musteriler.offline') }}" class="btn btn-outline-secondary">
...
<div class="row mb-4">
    <div class="col-md-4">
        <div class="stat-card">
            <div class="stat-card-title">Offline Müşteri Sayısı</div>
            <div class="stat-card-value text-danger">{{ $offlineMusteriler->count() }}</div>
        </div>
    </div>
</div>
    ...
    <div class="card-body">
        ...
        <div class="table-responsive">
            <table class="table table-hover" id="offlineTable">
                ...
                <tbody>
                    @foreach($offlineMusteriler as $musteri)
                    <tr>
                        <td>
                        ...
                        <td>
                            @if($musteri->son_baglanti_tarihi)
                                {{ \Carbon\Carbon::parse($musteri->son_baglanti_tarihi)->format('d.m.Y H:i') }}
                                <br>
                                <small class="text-muted">{{ \Carbon\Carbon::parse($musteri->son_baglanti_tarihi)->diffForHumans() }}</small>
                            @else
                        ...
                        <td>
                            <div class="btn-group btn-group-sm">
                                ...
                                <button class="btn btn-outline-success" onclick="internetAc({{ $musteri->id }})" title="İnternet Aç">
        ...
        <div class="text-center py-5">
            ...
            <h5 class="text-muted">Tüm müşteriler online!</h5>
            <p class="text-muted">Şu anda offline müşteri bulunmuyor.</p>
        </div>
        @endif
    </div>
</div>
...
<script>
    ...
    function internetAc(id) {
        Swal.fire({
            title: 'İnternet Açma',
            text: 'Bu müşterinin internetini açmak istediğinize emin misiniz?',
            icon: 'question',
